...
public protocol YLPhotoBrowserDelegate: class {
        ...
        if dataArray.keys.contains(index) {
            ...
            if let photo = delegate?.epPhotoBrowserGetPhotoByCurrentIndex(index) {
                ...
                if dataArray.count > 5 {
                    ...
                    let keysFirst = keys.first!
                    let keysLast = keys.last!
                    let first = abs(keysFirst - index)
                    let next = abs(keysLast - index)
                    if  first > next {
                        dataArray.removeValue(forKey: keysFirst)
                    }
                }
                return photo
            } else {
